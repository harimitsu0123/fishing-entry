...
window.handleRegistration = async function() {
    ...
    const entryData = {
        ...
        lastModified: existingEntry ? new Date().toISOString() : null
    };
                            ...
                            ${group.parts.map(m => `
                                <div class="drag-item-person" draggable="true" ondragstart="dragPerson(event, '${group.entry.id}', ${m.idx})">
                                    <div style="display:flex; align-items:center; gap:4px;">
                                        <button class="btn-leader-toggle ${m.p.isLeader ? 'active' : ''}" onclick="window.toggleLeader('${group.entry.id}', ${m.idx})">箝・/button>
                                        <span>${m.p.name}</span>
                                    </div>
                                </div>
                            `).join('')}
